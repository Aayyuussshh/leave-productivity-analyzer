  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        try {
          // ✅ Trim the employee code to remove any extra spaces
          const employeeCode = (rowObj["Employee ID"] || "").trim();
          ...
          // ✅ Use TRIM in the SQL query to handle any extra spaces in the database
          const [[emp]] = await connection.query(
            "SELECT id FROM employees WHERE TRIM(employee_code) = ?",
            [employeeCode]
          );
